...
ui <- dashboardPage(
  ...
  dashboardSidebar( # Side bar where we will put the selection widgets
    ...
    dateRangeInput(
      inputId = "dateRange", # The 'inputId' is used to refer shiny to the values selected by the user
    ...
    selectInput( #' selectInput' is a simple widget which allows single value selection
      ...
      selected = "Depth"
    ) # What is selected as default  when the app opens
  ),
  ...
  dashboardBody(
    fluidRow(
      ...
      box(
        width = 20, # Box is used to define the size of the map
      ...
      ) ##' plotOutput' is called to plot the ggplot. NOTICE: "myplot" is an element defined in the server side
    )
  )
)
...
server <- function(input, output, session) {
  ...
  ## First observe and subset the data according to user selection
  # reactive expression
  depth_reactive <- reactive({
    ...
    ## This we want to store into the reactive element
    subseted_data
  })
  ...
  output$mymap <- renderLeaflet({ # Here we create the leaflet map. NOTICE: 1. The map output name will be 'mymap' as defined by "output$mymap" 2. renderLeaflet - is a shiny 'function' which output is a leaflet map
    ...
    ## We use the subseted data from the reactive element 'depth_reactive()'
    leaflet_plot(data_in = depth_reactive(), color_by = color_by)
  })
  ...
  output$myplot <- renderPlot({ # Here we create the ggplot. NOTICE: 1. The map output name will be 'myplot' as defined by "output$myplot" 2. renderPlot - is a shiny 'function' which output is a plot
    ...
    ## We use the subseted data from the reactive element 'depth_reactive()'
    ggplot_plot(depth_reactive(), group_by = plot_by)
  })
}
